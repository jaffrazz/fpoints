...
class PelanggaranController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->loadAll(Yii::$app->request->post()) && $Sp->loadAll(Yii::$app->request->post()) && $model->save()) {
            $check = $this->checkDetailPoint($model, 'create');
            $checkdetail=DetailPoint::find()->where(['id_siswa' => $model->id_siswa])->one();
            $spke=Sp::find()->where(['id_siswa' => $model->id_siswa])->count();
            if(!is_null($checkdetail)){
                $point_pelanggaran=$checkdetail->point_pelanggaran;
                //$spplus=$spke->sp_ke;
                if(is_null($spke)){
                       
                    $Sp = new Sp();
                    $Sp->id_siswa = $model->id_siswa;
                    $Sp->sp_ke=1;
                    $Sp->tanggal_sp=$model->tanggal;
                    $Sp->jml_point_pelanggaran=$point_pelanggaran;
                    $Sp->save(false);
            }
            else if($point_pelanggaran >=100){
                // $spplus=$spke->count();
                $Sp = new Sp();
                $Sp->id_siswa = $model->id_siswa;
                $Sp->sp_ke=$spke+1;
